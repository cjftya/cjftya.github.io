...
export function buildMS2(collector: ModelCollector, quality: 'high' | 'low'): void {
  ...
  addCapsomers(collector, quality, quality === 'high' ? 72 : 38, 1.99, 'capsomer');
...
export function buildAdenovirus(
  ...
  addCapsomers(collector, quality, quality === 'high' ? 86 : 42, 1.94, 'capsomer');

  const vertices = getIcosahedronVertices().map((vertex) =>
  ...
  pentonUnits.mesh.name = 'adenovirus-vertex-pentons';
  ...
  fibers.name = 'adenovirus-fiber-shafts-and-knobs';
...
function addCapsomers(
  ...
  units.mesh.name = 'ordered-icosahedral-capsomers';
