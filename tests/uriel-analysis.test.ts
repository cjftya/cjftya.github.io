...
import {
  pointForNumber,
  metricsForNumbers,
} from "../src/uriel/analysis/geometry";
...
describe("Uriel geometry", () => {
  it("maps the pick board to a seven by seven coordinate system", () => {
    expect(pointForNumber(1, "board")).toEqual({
      number: 1,
      x: -0.86,
      y: -0.86,
    });
    expect(pointForNumber(7, "board")).toEqual({
      number: 7,
      x: 0.86,
      y: -0.86,
    });
...
describe("Uriel data and candidate search", () => {
  ...
  it("can expose one hundred distinct candidates", () => {
    ...
    expect(
      new Set(candidates.map(({ numbers }) => numbers.join("-"))).size,
    ).toBe(100);
    ...
    expect(
      new Set(candidates.flatMap(({ numbers }) => numbers)).size,
    ).toBeGreaterThan(40);
  });
  ...
  it("trains the ridge transition only on known history and layers the portfolio", () => {
    ...
    const second = findShapeCandidates(
      changedFuture,
      89,
      "board",
      100,
  ...
  it("keeps the previous numerical model as an isolated baseline", () => {
    ...
    expect(baseline.candidates.every(({ tier }) => tier === undefined)).toBe(
      true,
    );
  });
  ...
  it("keeps the selected model signal in ten role-balanced purchase games", () => {
    ...
    expect(
      new Set(purchase.games.map(({ numbers }) => numbers.join("-"))).size,
    ).toBe(10);
    expect(
    ...
    expect(
      hypothesisGames.every(({ reason }) => reason.includes("선택 모델")),
    ).toBe(true);
  });
  ...
  it("keeps a direct shape selection as the tenth purchase game", () => {
    ...
    const purchase = buildPurchasePortfolio(
      research.candidates,
      "circle",
      selected,
    );
  ...
  it("optimizes the selected research pool without inventing combinations", () => {
    ...
    expect(
      purchase.games.every(({ numbers }) =>
        researchKeys.has(numbers.join("-")),
      ),
    ).toBe(true);
    expect(purchase.optimizedScenarioCount).toBe(80);
    expect(purchase.topTenRetained).toBeGreaterThan(0);
  ...
  it("exposes the sequence shape model as a separate candidate experiment", () => {
    ...
    const result = findShapeCandidates(
      history,
      34,
      "board",
      100,
