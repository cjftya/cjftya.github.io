class ResourceLoader {
    ...
    __loadBackground() {
        ...
        const worker = new Worker("scripts/common/util/framework/resourcemanger/resourceworker.js");
        const imgElements = document.querySelectorAll('img[data-src]');
        imgElements.forEach(imageElement => {
            var imgUrl = imageElement.getAttribute('data-src');
            if (this.__typeMap[ThreadType.Background].get(imgUrl) != null) {
                worker.postMessage({ imgUrl: imgUrl, requestUrl: new URL(imgUrl, window.location.href).href });
            }
        })
        ...
        worker.addEventListener('message', event => {
            ...
            imageElement.onload = () => {
                ...
                if (imageData.imgUrl.indexOf("/viewer/") < 0) {
